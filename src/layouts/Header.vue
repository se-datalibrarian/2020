<template>
  <header class="fixed z-0 top-0 w-screen h-16 flex flex-wrap items-center justify-between lg:justify-evenly pl-2 pr-6 mb-8 header bg-gray-800">
    <g-link class="flex items-center" to="/" active-class="null">
      <LogoSymbol class="fill-current text-green-400 hidden md:inline-block h-4 mr-2" aria-label="Home" />
      <LogoTriangle class="fill-current text-green-400 inline-block md:hidden h-12" aria-label="Home" />
      <span class="hidden md:inline text-gray-100">{{ $static.metadata.siteName }}</span>
    </g-link>
    <nav class="hidden sm:inline">
      <g-link v-for="page in $static.allPage" :key="page.path" class="text-gray-100 sm:ml-4 transition duration-200 ease-in-out hover:text-green-400" :to="page.path">{{ formatPageLinks(page.path) }}</g-link>
    </nav>
    ...
    <transition name="slide-down">
      <nav v-if="show" class="mobile-nav absolute sm:hidden flex flex-col bg-gray-800 p-4 rounded-b-lg">
        <g-link v-for="page in $static.allPage" :key="page.path" v-on:click="show = false" class="text-xl text-gray-100 transition duration-200 ease-in-out hover:text-green-400" :to="page.path">{{ formatPageLinks(page.path) }}</g-link>
      </nav>
    </transition>
  </header>
</template>
...
export default {
  ...
  methods: {
    formatPageLinks: function(page) {
      ...
      return lowerCasePage.charAt(0).toUpperCase() +
